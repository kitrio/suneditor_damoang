...
foreach ($_FILES as $temp) {
    //파일명 변경
    $upload_filename = cut_str(md5(sha1($_SERVER['REMOTE_ADDR'])), 5, '-') . uniqid() . '-' . replace_filename($temp['name']);

    //이미지 SERVER PATH
    $filetowrite = $imageFolder . $upload_filename;
    //이미지 URL
    $imageurl_full = $imageurl . $upload_filename;
    ...
    $fileInfo->name = (string) $upload_filename;
    ...
    array_push($result, array('url' => $imageurl_full, 'name' => $upload_filename, 'size' => $size));
}
